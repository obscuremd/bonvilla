...
import { addToCart } from "@/app/(main)/cart/page";
...
export default function ProductCard({ product }: { product: ProductData }) {
  if (
    ...
    return (
      <div className="relative w-[170px] sm:w-[210px] md:w-[230px] flex-shrink-0">
        <div className="w-full h-[240px] sm:h-[280px] md:h-[300px] rounded-2xl bg-[#faf8f5] animate-pulse" />
        <div className="pt-2.5 space-y-2">
          <div className="h-3 bg-[#faf8f5] rounded w-3/4" />
  ...
  function handleAddToBag(e: React.MouseEvent) {
    ...
    // Add first available size (or prompt user to go to product page)
  ...
  return (
    <>
          ...
          <div className="relative w-full h-[240px] sm:h-[280px] md:h-[300px] rounded-2xl overflow-hidden bg-[#faf8f5]">
            ...
            >
              <button
                ...
                className={`w-full flex items-center justify-center gap-1.5 font-body font-semibold text-[10px] sm:text-xs py-2 sm:py-2.5 rounded-xl transition-all duration-200 shadow-lg ${
                  addedFeedback
                    ? "bg-green-600 text-white shadow-green-600/25"
                    : "bg-[#5b1619] hover:bg-[#4a1113] text-white shadow-[#5b1619]/25"
                }`}
              >
                <ShoppingBag size={11} />
      ...
      {/* Auth dialog — shown when unauthenticated user tries to add to bag */}
      <AuthDialog
        open={authOpen}
